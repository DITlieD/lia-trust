...
Install the current stable release (`v0.2.2`):
...
unavailable or the platform is unsupported, `auto` mode builds the same `v0.2.2` tag from source;
...
1. Install verified `lia 0.2.2` from the `v0.2.2` release, or build that version from source
...
./target/release/lia --version  # lia 0.2.2
./target/release/lia install --apply-live
```

The [`v0.2.2` release](https://github.com/DITlieD/lia-trust/releases/tag/v0.2.2) ships a verified
