...
Install the current stable release (`v0.3.0`):
...
unavailable or the platform is unsupported, `auto` mode builds the same `v0.3.0` tag from source;
...
1. Install verified `lia 0.3.0` from the `v0.3.0` release, or build that version from source
...
./target/release/lia --version  # lia 0.3.0
./target/release/lia install --apply-live
```

The [`v0.3.0` release](https://github.com/DITlieD/lia-trust/releases/tag/v0.3.0) ships a verified
